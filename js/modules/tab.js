...
layui.define(['element'], function(exports) {
	...
	var mod_name = 'tab',
		...
		Tab = function() {
			this.config = {
				...
				autoRefresh: true,   ///自动刷新  false:关闭自动刷新   true:开启自动刷新
				contextMenu:false
			};
		};
	var ELEM = {};
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			...
			globalTabIdIndex++;
			var content = '<iframe class="iframeBox" frameborder="0" src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
			var title = '';
			...
			if(_config.closed) {
				title += '<i class="tab-icon layui-unselect layui-tab-close" data-id="' + globalTabIdIndex + '"></i>';
			}
			//添加tab
            ...
            ELEM.contentBox.find('.layui-tab-item').each(function() {
                $(this).height(ELEM.contentBox.height());
            });
			if(_config.closed) {
				//监听关闭事件
			...
			element.tabChange(ELEM.tabFilter, that.getTabId(data.title));
			//自动刷新
            //console.log(_config.autoRefresh);
			if(_config.autoRefresh) {
                //console.log('a'+':'+ _config.autoRefresh+':'+tabIndex);
